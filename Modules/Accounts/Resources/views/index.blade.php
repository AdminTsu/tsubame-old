...
<div class="content-wrapper">
    <form id='accounts' class="container" action="{{ route('accounts.update') }}" @submit.stop.prevent='submit($event)'>
        {{ csrf_field() }}
        {{ method_field('PUT') }}
        <div class="row">
        <div class="col-12">    
...
<script>
    $(document).ready(function(){
        @if (request()->register == 'success')
            var url = window.open("", '_blank');
            url.location = "https://docs.google.com/forms/d/e/1FAIpQLSfDTjYy3tR5tSi0yaJDMhpZYYJmPoRMh-zvcP_tcqvn-aFWEw/viewform";
            window.location.href = "{{ route('accounts.index.id') }}";
        @endif
    });
</script>
@endsection
